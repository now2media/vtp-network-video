...
static enum video_format convert_vtp_format_to_obs(vtp_pixel_format_t format,
...
static void video_receive_loop(vtp_source_data *data) {
  ...
  while (data->running) {
    ...
    if (vtp_receiver_receive_frame(data->receiver, &frame)) {
      ...
      enum video_format obs_fmt =
          convert_vtp_format_to_obs(frame.format, needs_conversion);
      ...
      if (needs_conversion) {
        ...
        obs_frame.linesize[0] =
            frame.width * ((frame.format == VTP_FORMAT_RGBA ||
                            frame.format == VTP_FORMAT_BGRA)
                               ? 4
                               : 3);
      }
...
static void audio_receive_loop(vtp_source_data *data) {
  ...
  while (data->running) {
    ...
    if (vtp_receiver_receive_audio(data->receiver, &audio_frame)) {
      ...
      obs_audio.speakers =
          (audio_frame.channels == 2) ? SPEAKERS_STEREO : SPEAKERS_MONO;
...
static const char *vtp_get_name(void *) { return "VTP Client Source"; }
...
static void vtp_destroy(void *priv_data) {
  ...
  if (data) {
    ...
    if (data->audio_thread.joinable())
      data->audio_thread.join();
...
static void vtp_update(void *priv_data, obs_data_t *settings) {
  ...
  if (data->use_discovery && !data->stream_name.empty()) {
    ...
    bool connected = vtp_receiver_connect_by_name(data->receiver, listener,
                                                  data->stream_name.c_str());
    if (!connected) {
      std::cerr << "Discovery connection failed: " << data->stream_name
                << std::endl;
    }
...
static obs_properties_t *vtp_properties(void *) {
  ...
  obs_properties_add_text(props, "stream_name", "Stream Name",
                          OBS_TEXT_DEFAULT);

  obs_properties_add_text(props, "ip", "Server IP", OBS_TEXT_DEFAULT);
  obs_properties_add_int(props, "port", "Server Port", 1024, 65535, 1);
...
static void vtp_defaults(obs_data_t *settings) {
  ...
  obs_data_set_default_string(settings, "stream_name", "VTP-cam-1");
