...
struct vtp_output_data {
  ...
  std::string stream_name = "vtp-output";
  std::atomic<bool> active{false};
};

static const char *vtp_output_get_name(void *) { return "VTP Send Output"; }
...
static bool vtp_output_start(void *priv_data) {
  ...
  data->sender = vtp_create_sender(data->stream_name.c_str(), width, height,
                                   fps, false, true);
  if (!data->sender || !vtp_sender_start(data->sender)) {
    std::cerr << "[vtp-network-video Output] Failed to start VTP Sender"
              << std::endl;
    if (data->sender) {
      vtp_destroy_sender(data->sender);
      data->sender = nullptr;
  ...
  std::cout << "port: " << vtp_sender_get_port(data->sender) << std::endl;
  return true;
}
...
static void vtp_output_raw_video(void *priv_data, struct video_data *frame) {
  auto *data = static_cast<vtp_output_data *>(priv_data);
  if (!data || !data->active || !data->sender)
    return;

  vtp_sender_send_frame(data->sender, frame->data[0], VTP_FORMAT_RGBA,
                        frame->timestamp);
}
...
static void vtp_output_raw_audio(void *priv_data, struct audio_data *audio) {
  ...
  vtp_sender_send_audio(data->sender, audio->data[0], (int)audio->frames, 48000,
                        2, audio->timestamp);
}

static void vtp_output_defaults(obs_data_t *settings) {
  obs_data_set_default_string(settings, "stream_name", "vtp-output");
}

struct obs_output_info vtp_output_info = {.id = "vtp_output",
...
void vtp_output_start_global(const char *name) {
  ...
  global_vtp_output =
      obs_output_create("vtp_output", "VTP Output", settings, nullptr);
